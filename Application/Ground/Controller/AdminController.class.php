...
class AdminController extends Controller {
	...
	protected function _initialize() {
        ...
        if(UID){
            ...
            //场馆禁用
            $venue['status'] or $this->error('场馆被禁用，禁止访问','Index/Index/index',5);
            $services = $venue['services'] ? explode(',',$venue['services']):[];
            $all_services = ['免费停车场','VIP休息室','柜子租赁','活动区'];

            if(!empty($services)){
                $my_services = [];
                foreach($all_services as $v){
                    if(in_array($v,$services)){
                        $my_services[] = ['val'=>$v,'s'=>'selected'];
                    }else{
                        $my_services[] = ['val'=>$v,'s'=>''];
                    }
                }
            ...
            define('NAME', $venue['name']);
            define('VID', $venue['id']);
            define('STARS', $venue['stars']);

            $venue['update_time'] and $this->setA = true;
            $items = M('VenueItems')->where(['vid'=>VID])->count();
            $items and $this->setB = true;
            $blocks = M('VenueBlock')->where(['vid'=>VID])->count();
            $blocks and $this->setC = true;

            $orders = M('Orders')->where(['vid'=>VID])->count();
            $this->orders = $orders?$orders:0;
            $locks = M('Locks')->where(['vid'=>VID])->count();
            $this->locks = $locks?$locks:0;
            $settled = M('Orders')->where(['vid'=>VID,'status'=>8])->count();
            $this->settled = $settled?$settled:0;

            $this->icon = D('VenueAlbumView')->where(['vid'=>VID,'status'=>1,'types'=>1])->find();
        }
		if (!UID) {// 还没登录 跳转到登录页面
        ...
        $material = C('MATERIAL');
        $light = C('LIGHT');
        $venue['material'] = $material[$venue['material']];
        $venue['light'] = $light[$venue['light']];
        $this->venue = $venue;
