...
class NeedsApi {
     ...
     * @author waco <etoupcom@126.com>
     */
    public static function batchDataGuzhi($nid,$aid,$status){
        if(empty($nid) or empty($aid) or empty($status)){
            return false;
        }
        $info = M('MainNeeds')->find($nid);
        ...
        switch($status){
            case 8:
                //调整需求状态
                self::doMainNeedsGuzhi($info,$status);
                //需求账户关系
                self::doNeedsAccount($nid,$aid);
     ...
     * @author waco <etoupcom@126.com>
     */
    public static function doMainNeedsGuzhi($info,$status){
        $back = M('MainNeeds')->save(['id'=>$info['id'],'status'=>$status,'begin_trading'=>NOW_TIME]);
        return $back?true:false;
    }
     ...
     * @author waco <etoupcom@126.com>
     */
    public static function doStatisticsTake($data,$hasnum = false,$begin=0){
        ...
        if($hasnum){
            //获取账户信息
            $needsaccount = M('NeedsAccount')->where(['nid'=>$data['nid']])->find();
            $diffDate = self::timediff($begin,NOW_TIME);
            $days = intval($diffDate['day']+1);
            if($days){
                $money = (($data['interest_rate']-$data['benchmark'])/100*$data['with_funds']*10000*0.1)*$days/30;
            }
        }else{
            //获取账户信息
