...
class BAOFOOApi {
	...
	public function postData() {
		...
		$back = D('MainRecord')->saveData($OrderMoney,$data['TransID'],'BAOFOO');
        if ($back) {
            //站内信
            $title = '您通过宝付支付在线操作充值';
            $user_con = time_format() . '您通过宝付支付在线操作充值，金额：' . money($OrderMoney);
            $admin_con = time_format() . '用户通过宝付支付在线操作充值，用户名：' . USERNAME . '，金额：' . money($OrderMoney) . '，单号：' . $data['TransID'] . '，<a href="'.C('HTTPHOST').'/admin.php/Record/all">立即查看</a>';
