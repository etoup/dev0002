	...
	if (!empty($grid['href'])) {
		...
		foreach ($links as $link) {
			...
			if (preg_match('/^\[([a-z_]+)\]$/', $href, $matches)) {
				...
				switch ($show) {
					case '删除':
						$val[] = '<a href="'.U($href).'" class="mr10 J_ajax_del">'.$show.'</a>';
						break;

					default:
						$val[] = '<a href="'.U($href).'" class="mr10">'.$show.'</a>';
						break;
				}
			}
		}
...
function traverse($path = '.') {
	...
	$pathArr     = array();
	if ($current_dir) {
		if (false !== ($dh = opendir($current_dir))) {
