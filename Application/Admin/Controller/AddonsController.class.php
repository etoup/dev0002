...
class AddonsController extends AdminController {
	...
	public function uninstall() {
		...
		if (!$uninstall_flag) {
			ajaxMsg('执行插件预卸载操作失败'.session('addons_uninstall_error'), false);
		}

		$hooks_update = D('Hooks')->removeHooks($db_addons['name']);
		if ($hooks_update === false) {
			ajaxMsg('卸载插件所挂载的钩子数据失败', false);
		}
		S('hooks', null);
		$delete = $addonsModel->where("name='{$db_addons['name']}'")->delete();
		if ($delete === false) {
			ajaxMsg('卸载插件失败', false);
		} else {
			ajaxMsg('卸载成功');
		}
	}
	...
	public function hooks() {
		$map                            = $fields                            = array();
		$keyword                        = I('keyword', '');
		$keyword && $map['description'] = array('like', '%'.$keyword.'%');
		//p($map, 0);
...
;
{$extend}public function install(){
            return true;
        }
