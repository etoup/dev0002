...
class CategoryModel extends Model {
	...
	public function update($data) {
		$data = isset($data)?$this->create($data):$this->create();
		if (!$data) {//数据对象创建错误
			return false;
		}
		/* 添加或更新数据 */
	...
	public function getList($where = null) {
		if (isset($where)) {
			$list = $this->field('id,pid,title,name,sort,mold,model,description,check')->order('sort,id')->where($where)->select();
		} else {

			$list = $this->field('id,pid,title,name,sort,mold,model,description,check')->order('sort,id')->select();
		}
	 ...
	 * @return array
	 */
	public function getMap($where = array()) {
		if (!isset($_map)) {
			if (empty($where)) {
				$list = $this->getList();
			} else {
	...
	public function getNodeByLevel($parentid, $map) {
		...
		foreach ($map[$parentid] as $key => $value) {
			...
			$value['modelArr'] = empty($value['model'])?array():explode(',', $value['model']);
			$array[]           = $value;
