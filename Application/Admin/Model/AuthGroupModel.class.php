...
class AuthGroupModel extends Model {
	 ...
	 *
	 * 示例: 把uid=1的用户添加到group_id为1,2的组 `AuthGroupModel->saveToGroup(1,'1,2');`
	 */
	public function saveToGroup($uid, $gid) {
		$uid = is_array($uid)?implode(',', $uid):trim($uid, ',');
		$gid = is_array($gid)?$gid:explode(',', trim($gid, ','));
     ...
     * @author waco <etoupcom@126.com>
     */
    static public function removeByUid($uid){
        return M(self::AUTH_GROUP_ACCESS)->where(array('uid'=>$uid))->delete();
