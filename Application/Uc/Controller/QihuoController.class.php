...
class QihuoController extends UcenterController {

	/**
	 * 配资管理
	 */
	public function index() {
        $map = ['uid'=>UID,'types'=>['gt',0]];
    ...
    public function early(){
        if(IS_POST){
            ...
            if($this->_checkPwd($paypwd,true)){
                ...
                $admin_con = time_format().'用户提交结束申请【股指吧】，用户名：'.USERNAME.'，单号：'.$orders.'，<a href="'.C('HTTPHOST').'/admin.php/Needs/early">立即查看</a>';
                D('MessageNotices')->saveData(UID,$title,$user_con,1,$admin_con);
                $this->success('申请成功，等待审核');
            }else{
                $this->error('支付密码错误');
