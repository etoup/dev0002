...
class DoqihuoController extends UcenterController {
	...
	public function index() {
        ...
        if($needs){
            ...
            //利息
            $interest_rate = C('QIHUOINTERSTRATE')?C('QIHUOINTERSTRATE'):2.00;
            ...
            //预警线
            $needs['warning_val'] = C('QIHUOWARNINGLINE')?C('QIHUOWARNINGLINE'):50;
            $needs['warning_funds'] = $needs['with_funds']*$needs['warning_val']/100;
            //平仓线
            $needs['open_val'] = C('QIHUOOPENLINE')?C('QIHUOOPENLINE'):30;
            $needs['open_funds'] = $needs['with_funds']*$needs['open_val']/100;
            //月利率
            $needs['interest_rate'] = C('QIHUOINTERSTRATE')?C('QIHUOINTERSTRATE'):2.00;
            ...
            switch($from){
                ...
                case 'peizi':
                    ...
                    if (!UID) {
                        ...
                        //缓存
                        $data = I('post.');
                        $data['interest_rate'] = C('QIHUOINTERSTRATE')?C('QIHUOINTERSTRATE'):2.00;
                        $data['interest_rate'] = RATE?RATE:C('QIHUOINTERSTRATE');
    ...
    public function doneeds() {
        ...
        $needs = self::_getNeeds();
        if($needs){
            $warning_val = C('QIHUOWARNINGLINE')?C('QIHUOWARNINGLINE'):50;
            $open_val = C('QIHUOOPENLINE')?C('QIHUOOPENLINE'):30;
            ...
            $needs['warning_line'] = round(intval($needs['own_funds'])*$warning_val/100+$needs['with_funds'],2);
            $needs['warning_line_format'] = money($needs['warning_line']*10000);
            $needs['open_line'] = round(intval($needs['own_funds'])*$open_val/100+$needs['with_funds'],2);
